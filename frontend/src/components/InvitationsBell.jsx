...
function InvitationsBell({ onAccepted }) {
  ...
  const respond = async (invitationId, action) => {
    ...
    try {
      const res = await authFetch(`/api/invitations/${invitationId}/respond`, {
        method: 'POST',
    ...
    <div className="relative" ref={wrapperRef}>
      ...
      {open && (
        <div
          ...
          style={{ background: 'var(--surface)', border: '1px solid var(--border)', animationDuration: '0.15s' }}
        >
          <p className="text-xs font-semibold px-1 mb-2" style={{ color: 'var(--text-muted)' }}>
            Board invitations
          </p>

          {invitations.length === 0 && (
            <p className="text-sm px-1 py-3" style={{ color: 'var(--text-muted)' }}>
            ...
            {invitations.map((inv) => (
              ...
              >
                <p className="text-sm mb-2" style={{ color: 'var(--text)' }}>
                  <span className="font-semibold">{inv.invitedBy?.name || 'Someone'}</span> invited you to{' '}
                  <span className="font-semibold">{inv.board?.name || 'a board'}</span>
                </p>
