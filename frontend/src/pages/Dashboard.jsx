...
function StatCard({ label, value, accent }) {
  return (
    ...
    >
      ...
      <p className="font-display text-2xl font-bold" style={{ color: accent || 'var(--text)' }}>{value}</p>
    </div>
  );
}
      ...
      <div className="relative z-0 max-w-5xl mx-auto px-6 py-14 fade-in-up">
        ...
        {stats && (
          <div className="flex flex-wrap gap-4 mb-10">
            ...
            <StatCard label="Completion Rate" value={`${stats.completionRate}%`} accent="var(--accent-2)" />
          </div>
        )}
